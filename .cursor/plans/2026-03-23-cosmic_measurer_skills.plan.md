...
3. **Canonical FP exit** — Every measured functional process must **append** one final **X**: `**Errors/notifications`** (`dataGroupRef`: `status/errors/etc`, last `order`), after any artifact-specific movements (including parser `return` exits for Apex). See [reference.md](../skills/cosmic-measurer/reference.md) § Canonical FP exit. Apex implements this in `movements.py` `build_output`; other artifact skills must match the same appended row (adjust `implementationType` per artifact).
